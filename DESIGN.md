...
src/
  ...
  app/
    ...
    useStartDesign.ts      首页「开始设计」流程 hook（校验→refine 重试（携带参考图）→handoff（含原始 prompt）→跳转；红色 LLM 错误行：缺设置/全部 JSON 重试失败持久到下次提交、重试中 5 秒自动消失；LLM 请求失败与系统提示词资产加载失败走红浮窗 ErrorFloat，5 秒自动消失，见「requestError.ts」节）
    ...
    components/
      ...
      ResolutionPicker.tsx  首页长宽比+分辨率选择器（props 驱动；Custom=预设比例 pill 行 + W/H 输入（联动逻辑在内）、Official=官方比例/分辨率两行 pill 无 W/H 输入，见「index.tsx」节）
      ...
      ErrorFloat.tsx       红色错误浮窗：useErrorFloat()（show(message)，5 秒自动消失、新消息重置计时）+ ErrorFloat 组件（portal 到 document.body、pointerEvents 关闭、视口顶部居中，testID error-float）——所有网络请求失败/资产加载失败/下载反馈的统一出口（首页两个实例：开始设计流程 + 示例集加载；设计页一个实例：生成/下载），见「requestError.ts」节
      ...
      design/              设计页展示组件（props 驱动，无自身状态；例外 BackgroundEditor 自持对话框开关/草稿态）
        ...
        HeaderBackButton.tsx  Stack 页头返回按钮（页头标题已清空 `title: ''`，headerLeft 注入；刷新后仍可用，见「页面与导航」节）
    ...
    services/
      ...
      imageDownload.ts     downloadImage：fetch 图片（非 2xx 抛 `HttpError`，网络层失败透传 TypeError）→ blob → 临时 `<a download>` 点击触发浏览器保存
      ...
      resolutions.ts       长宽比/分辨率常量与纯函数：官方固定分辨率集合（36 个）+ 按约分比例分组（23 组，比值横→竖排序、组内大者在前 `OFFICIAL_RATIO_GROUPS`）+ `ratioLabel`（gcd 约分标签）+ `pickOfficialSize`（吸附：精确匹配 > 同比例最近面积 > 最近 1:1）+ Custom 预设比例 `PRESET_RATIOS`，见「index.tsx」节
      requestError.ts      错误分类与友好文案：HttpError（非 2xx，保留 status）/ AssetLoadError（打包资产加载失败，cause 保留原始错误）/ classifyRequestError（401/403→auth、404→notFound、其他 4xx→rejected、5xx→server、其余→unexpected；网络层 TypeError/网络措辞消息→unreachable；SyntaxError→unexpected；非请求错误→null）/ requestErrorMessage（服务名 + 错误类别 → i18n 友好文案，区分「网络问题」与「配置问题」，不泄漏 Failed to fetch 等原始文字），见「requestError.ts」节
      ...
      exampleCollection.ts 首页示例集：fetch public/example_collection/example.json（`loadExampleCollection` 解析为 `{ entries, error }`——集合级失败 entries 为 []、error 为 HttpError/原始错误，供首页走红浮窗 `examplesLoadFailed`；单张图失败仍静默 `imageId=null`），示例图先持久化进 IndexedDB 图片库（稳定 id，check-then-store）+ 映射为 Design 形状（sizeFromRatio / exampleToDesign），见「index.tsx」节
    ...
    test/
      services/            jest 单测（PromptRefiner、IdeogramPrompt、settings、designStore、imageDownload、imageStore、publicAsset、requestError、resolutions）
...
- 图片生成：两种 provider 都直接请求 `{base}/v1/ideogram-v4/generate`；Official 直接请求**裸路径** `/v1/ideogram-v4/generate`（**无任何 host 前缀**，设置中只读显示、解析到应用自身源）——浏览器无法跨域调用官方 API，实际使用时把 **CORS 代理**（独立的 cf-worker 项目，镜像官方 API + 补 CORS 头 + 图片代理）填进 **Custom** 端点；Custom 用本地/私有化部署的兼容端点（默认 `http://127.0.0.1:8000`）；`Api-Key` 头仅在密钥非空时发送。
...
| Image generation provider | 下拉：Official / Custom | Official = Ideogram 4 官方 API（裸路径 `/v1/ideogram-v4/generate`，无前缀，见下）；Custom = 本地/私有化兼容端点（默认 `http://127.0.0.1:8000`），或镜像官方 API 的 CORS 代理（cf-worker 项目） |
| Image generation endpoint | 文本框（testID `settings-image-endpoint`） | **仅 Custom 可编辑**；Official **硬编码** `/v1/ideogram-v4/generate` 只读显示（无前缀、不可编辑，存储的端点值不会被它覆盖） |
...
- **覆盖**：首页（侧栏 Home/Recent Designs 导航、图片墙标题/空态、输入区标题、W/H/自定义、prompt 占位符、开始按钮无障碍标签（开始设计/处理中）、参考图删除按钮 `removeImage`、红色 LLM 错误行（缺设置 `missingSettingsAlert`、重试 `refineRetrying`、全部 JSON 重试失败 `refineAllFailed`）、示例集加载失败浮窗 `examplesLoadFailed`、红色浮窗网络文案（服务名 `netServiceLlm`/`netServiceImage`/`netServiceDownload` + 六类错误 `netUnreachable`「网络问题」/`netAuth`/`netNotFound`/`netRejected`「配置问题」/`netServer`/`netUnexpected`，均含 `{service}`/`{status}` 占位，见「requestError.ts」节））、设计页（元数据六标签 + 背景标签、网格/元素开关、画布占位、工具提示、Save/Generate/Download/Show Prompt、已保存、历史条「Generated (N)」、右键菜单、编辑对话框含字体区、背景编辑对话框标题（`editBackground`）、显示 Prompt 对话框两栏标签、调色盘 popover、设置对话框全部标签与按钮——设置对话框的保存按钮用独立 key `saveSettings`（en "Save Settings" / zh "保存设置"，不与设计页 Save 的 `save`（zh "保存设计"）混用）、生成/下载错误提示（缺设置、空元素、请求失败、无图可下、重写失败等）。
...
页面文件只做状态声明与组合；「开始设计」流程（设置校验 → refine 重试 → handoff → 跳转 + 红色 LLM 错误行）在 `useStartDesign.ts`（同设计页 hooks 的拆分模式，index.tsx 因此保持 <400 行）；长宽比/分辨率选择器是独立组件 `ResolutionPicker.tsx`（provider 相关，见下第 2 条）；下方图片墙是独立组件 `RecentDesignsWall.tsx`（见下）。
...
2. **长宽比 + 分辨率选择器**（`components/ResolutionPicker.tsx`，props 驱动——`selectedRatio`/`width`/`height` 状态与全部处理函数在页面，组件只渲染 + 回调）——**随 image generation provider 变化**（页面状态 `imageProvider` 挂载时从设置读取，设置对话框关闭后重读——Save 与 Cancel 都走 `onClose`）：
   - **Custom（现有逻辑不变）**：长宽比 pill 行（居中，窄屏换行）：`4:3, 3:4, 16:9, 16:10, 9:16, 10:16, 1:1, custom`（testID `ratio-<ratio>` / `ratio-custom`）+ W/H 行（居中；输入 testID `width-input` / `height-input`）。选非 custom 比例时：以 1024 为基准算出默认 W/H；随后在 W 或 H 输入数字，另一边按比例自动联动（取整）。选 custom 后 W/H 互不联动，长宽比取 `${W}:${H}`。
   - **Official（新增）**：分辨率只能从官方固定集合里选，**两行 pill、无 W/H 输入**——第一行是固定集合按约分长宽比分组的组（23 个，按比值横→竖排序，testID `official-ratio-<ratio>`），选中后第二行显示该长宽比下的 `WxH` 分辨率（每组 1–2 个，大者在前，testID `official-resolution-<W>x<H>`）。选分辨率即直接定 W/H（= handoff `size` 与生成时 `resolution=WxH`），`selectedRatio` 置为该分辨率的约分比例标签（LLM 目标长宽比）；点第一行某比例默认选中该组**最大**分辨率。固定集合与分组/吸附逻辑在 `services/resolutions.ts`（36 个官方分辨率 → 23 个比例；`OFFICIAL_RESOLUTIONS` / `OFFICIAL_RATIO_GROUPS` / `ratioLabel`（gcd 约分）/ `pickOfficialSize`（吸附：精确 > 同比例最近面积 > 最近 1:1；另含 Custom 用的 `PRESET_RATIOS`））。
   - **provider 切换适配**：切到 Official → 当前 W/H 经 `pickOfficialSize` 吸附进固定集合；切到 Custom → W/H 原样保留、比例置 `custom`（官方比例可能不在预设列表，选预设会触发 1024 基准重置 effect 把 W/H 冲掉）。页面首次挂载即 Official 时同样在 `useLayoutEffect`（首帧绘制前，无闪动）把默认 4:3 1024×768 吸附为 1152×864；「选预设 → 重置 1024 基准」effect 仅 Custom 模式运行。
3. （W/H 行已并入第 2 条选择器：仅 Custom 模式存在）
4. **参考图预览行**（有参考图时渲染；长宽比/分辨率选择器与 prompt 输入条之间，居中，间距 8）：每张参考图（拖入或粘贴）一张 48×48 缩略图（圆角 6、1px #DDD 边，testID `image-preview-<i>`），右上角 16px 半透明黑圆 × 按钮（lucide `X`，testID `image-preview-remove-<i>`，aria-label `removeImage`）删除该图（列表重新索引）。图片是**首页会话状态**（data URI 数组），不进 handoff/store，跳转后丢失。
...
6. `useStartDesign` 流程：prompt 非空且 LLM 设置项齐全（缺失则输入条上方**红色错误行**列出缺项并停止，不发起请求）→ `refine(system_prompt.txt, prompt, ratio, refImages)`（端点/密钥/模型名取自设置；`refImages` 为预览行里的参考图 data URI 数组，非空时以多模态格式进请求，见「PromptRefiner.ts」节）得到 JSON prompt 字符串。**JSON 格式校验 + 重试**：LLM（temperature 1.0）可能返回无法 `JSON.parse` 的回答，每次尝试后立即解析，失败时在输入条上方显示红色错误条（"retrying" 期间 **5 秒后自动消失**，新的失败重新计时）并重试，最多 `REFINE_MAX_ATTEMPTS = 3` 次，全部失败后错误条转为持久（不自动消失、下次提交才清除）不跳转。校验通过 → 生成新设计 id、把 prompt 与 W/H 写入 handoff（`setDesignHandoff`，见「页面与导航」节）→ URL 只带 `id` 跳 design 页。**失败分两类**：① **请求失败**（LLM 端点网络不可达、HTTP 非 2xx、system prompt 打包资产加载失败）→ 红色浮窗（`ErrorFloat`，testID `error-float`，5 秒自动消失），文案由 `requestErrorMessage`（见「requestError.ts」节）按错误类别生成——**网络问题**（"Network problem: can't reach the language model (LLM) service…"）与**配置问题**（401/403 凭据被拒、404 端点找不到、其他 4xx 请求被拒，均含状态码并指向 Settings）区分明确，**永不显示 `Failed to fetch` / `LLM API Error (500): …` 等原始文字**；② **非请求失败**（缺设置、全部 JSON 重试失败）→ 输入条上方红色错误行（testID `refine-error`），持久到下次提交。`Alert.alert` 在 RN-web 上是 **no-op**（浏览器收不到 dialog），所以 LLM 侧失败走浮窗或错误行而非 Alert；错误行样式：浅红底 + 1px #E53935 边框、文字 #E53935，位于输入条上方；每次提交开始时先清除上一轮错误。
...
- **Home 模式（默认）**：显示打包的**示例集**——`exampleCollection.ts` 挂载时 fetch `public/example_collection/example.json`（每项 = 原始 `prompt` + `jsonprompt` + 生成图 `url`），**先把每张示例图抓图持久化进 IndexedDB 图片库**（稳定 id = `img-<url 文件名>`，check-then-store：已在库则不重复抓，约 2MB base64 转换每浏览器只做一次；单张失败该条目 `imageId=null`、无 tile，不影响其余），再经 `exampleToDesign` 映射为 `Design` 形状（id 取 url 文件名、`images=[imageId]`（持久化失败为空数组）、`rawPrompt=prompt`、size 由 `aspect_ratio` 经 `sizeFromRatio` 换算，长边 1024 基线）；**显示标题 `collections`**（en "Collections" / zh "合集"，与 Recent 区标题同款样式，同时把图片墙与上方 prompt 输入区在视觉上隔开）；集合级加载失败（网络/非 2xx/非数组）回退空数组，显示 `noExamples` 空态（标题仍显示），**同时红浮窗显示 `examplesLoadFailed`**（"示例集加载失败，请刷新页面重试…"，5 秒自动消失；单张示例图持久化失败仍静默，只影响该 tile）。
...
5. **请求与结果**：POST 到图像生成 URL（`getImageUrl`：Official = 裸路径 `/v1/ideogram-v4/generate` 无前缀、解析到应用自身源，Custom = 用户端点 + 同一路径，见「设置」节），密钥非空时带 `Api-Key` 头；非 2xx → `HttpError`（带状态码）；取 `data[0].url` 后经 `resolveGeneratedImageUrl` 解析（根相对/相对 url——cf-worker 把 `data[].url` 重写为自身的 `/v1/ideogram-v4/image_proxy?url=…`——拼到图像服务端点 base，见「imageStore.ts」节）再经 `saveGeneratedImage` 转 base64 持久化（返回结果 union）——**持久化是唯一图片路径，无原始 URL 回退**：失败时该图被丢弃（不进 `images`、不进历史条），**抓取失败（网络/非 2xx）走红浮窗友好文案（服务=图片生成服务），IDB 写入失败才显示错误行 `imageSaveFailed`**（"Image generated, but saving it locally failed — try again."）；成功则把返回的 id 追加进 `images` 并把画布切到新图。生成中按钮显示 ActivityIndicator 并禁用。
6. **错误分流**（请求失败 → 红浮窗 `error-float`，非请求失败 → 按钮行下方红错误行 testID `generate-error`）：**bbox 改写 LLM** 的请求失败按服务「LLM」、**图像生成请求**（POST 非 2xx / 网络不可达）与**生成图抓取**的失败按服务「图片生成服务」，文案由 `requestErrorMessage`（见「requestError.ts」节）生成（网络问题 vs 配置问题，含状态码，永不显示 `Failed to fetch` / 状态码裸文）；**例外**：官方 Ideogram API 非 2xx 响应体带 `{"error": "<文案>"}`（字符串形态，如安全审核 "Prompt provided failed safety check…"）时，浮窗**原样显示该 error 字段内容**（`HttpError.detail` 优先，替换通用文案、不显示状态码）；**改写系统提示词资产加载失败** → 浮窗 `rewritePromptLoadFailed`；**非请求错误**（改写返回非法 JSON `rewriteNoJson`、缺 elements `rewriteMissingElements`、响应无图 URL `noImageUrl`、其他）→ 错误行显示其文字（兜底 `generationFailed`）。

### 下载图片（Download Image）
Save/Generate 行中 Generate 右侧的按钮，下载**画布当前选中**的那张生成图（历史条选中的缩略图，默认最新一张）：
- 有选中图 → 先 `resolveImageRef` 把 ref（IDB id）解析成可显示 uri（IDB 查 data URI；解析不到——含旧设计遗留的 URL ref——按「无图可下」处理）→ `downloadImage`（services/imageDownload.ts）fetch 该 uri（data URI 同样可 fetch）→ blob → 临时 `<a download>`（文件名 `{designId}.png`）点击触发浏览器保存窗口；下载中按钮显示 ActivityIndicator 并禁用；**fetch 失败走红浮窗友好文案**（`requestErrorMessage`，服务=「要下载的图片文件」：网络不可达→网络问题、非 2xx→配置/服务端问题含状态码，永不显示 `Failed to fetch`）。
- 还没有任何生成图（首页重写后未生成、或历史设计无图）→ 按钮仍可用，点击弹**红色浮窗**（`ErrorFloat`，testID `error-float`，视口顶部居中、`pointerEvents` 关闭）「No image has been generated yet — generate one first, then download.」，**5 秒后自动消失**（新的失败重新计时）。
...
### PromptRefiner.ts
两个 OpenAI 兼容 chat/completions 调用（共用内部 `chatCompletion`；端点/凭据/模型名在调用时从**当前激活的 provider profile** 读取，非 2xx 时抛 `HttpError`（message 含状态码+响应体、`status` 字段保留状态码），网络层失败原样透传浏览器 TypeError——上层经 `requestError` 分类生成浮窗文案，原始文字不进 UI）：
...
### requestError.ts
所有网络请求失败的统一分类与友好文案（用户必须能感知是**网络问题**还是**配置错误**，且界面永不出现 `Failed to fetch` 等原始浏览器文字）：
- `HttpError extends Error`：非 2xx 响应，`status` 字段保留状态码（message 保留原始细节仅供 console，不进 UI）；可选 `detail` = 上游服务自带的人类可读错误文案——**有 `detail` 时浮窗直接显示它（原样、不翻译），替换通用友好文案**（见下）。`PromptRefiner.chatCompletion`（LLM 非 2xx）、`imageDownload`（下载非 2xx）、`saveGeneratedImage`（抓取生成图非 2xx）、`loadExampleCollection`（example.json 非 2xx）、生成请求（useGeneration，非 2xx 时经 `upstreamErrorMessage` 提取官方 `{"error": "…"}` 体文案）都抛它。
- `upstreamErrorMessage(body)`：从非 2xx 响应体提取上游错误文案——官方 Ideogram API 失败时回 `{"error": "<原因>"}`（如安全审核 "Prompt provided failed safety check due to the inclusion of prohibited content."）；仅当 `error` 是**非空字符串**时返回（trim 后），非 JSON / 空串 / 对象形态（OpenAI 式 `{"error": {"message"}}`）一律返回 `null`（保留按状态码的通用文案）。
- `AssetLoadError extends Error`：打包的同源资产（`system_prompt.txt`、`system_prompt_rewrite_adapt_bbox.txt`）fetch 失败——无论网络层还是非 2xx，对用户都是「资源没打进去」，`cause` 保留原始错误；`useStartDesign.loadSystemPrompt` 与 `useGeneration.loadRewriteSystemPrompt` 捕获后抛它，UI 固定显示 `systemPromptLoadFailed` / `rewritePromptLoadFailed` 浮窗。
- `classifyRequestError(error)`：`HttpError` 按状态码分桶（401/403→`auth`、404→`notFound`、其他 4xx→`rejected`、5xx→`server`、其余→`unexpected`）；浏览器网络层 `TypeError`（或消息匹配 `failed to fetch`/`networkerror`/`load failed`）→`unreachable`；`SyntaxError`（响应体非法 JSON）→`unexpected`；**非请求错误（普通 Error/字符串/null）→ `null`**——调用方据此决定走红浮窗还是原有内联错误行。
- `requestErrorMessage(error, service, t)`：服务名（`llm`/`image`/`download` → `netServiceLlm`「the language model (LLM) service」/`netServiceImage`「the image generation service」/`netServiceDownload`「the image file」）+ 错误类别 → 六条 i18n 文案之一（`netUnreachable`「Network problem: … Check your internet connection, or verify the endpoint URL in Settings.」、`netAuth`/`netNotFound`/`netRejected`「Settings problem: … (status {status}) …」、`netServer`「… is temporarily unavailable (status {status}). Please try again in a moment.」、`netUnexpected`），`{status}` 仅在非 unreachable 时填入。
- 出口：`ErrorFloat`（components/ErrorFloat.tsx，testID `error-float`，5 秒自动消失）承载上述文案；**内联错误行**（首页 `refine-error`、设计页 `generate-error`）只保留非请求错误（缺设置、空元素、非法 JSON、无图 URL、IDB 写入失败等）。
...
- `getLlmUrl` 在 base（厂商或自建后端，去尾斜杠）后追加路径：Ollama 用原生 `LLM_OLLAMA_CHAT_PATH`（`/api/chat`），其余用 `LLM_CHAT_PATH`（`/chat/completions`）；`getImageBase` = Official → 空串（无前缀，其请求解析到应用自身源）、Custom → `imageEndpoint`（去尾斜杠）；`getImageUrl` = Official → 裸路径 `/v1/ideogram-v4/generate`（无前缀）、Custom → `getImageBase` + `/v1/ideogram-v4/generate`（端点 + 同一路径）。
...
- `resolveGeneratedImageUrl(url, endpointBase)`：解析生成图的抓取 URL——cf-worker 代理把 `data[].url` 重写为 **worker 自身的根相对路径**（`/v1/ideogram-v4/image_proxy?url=…`），浏览器会把根相对路径解析到 **app 自己的 origin**（静态托管源，错的主机），所以相对/根相对 url 拼到图像服务端点 base（`getImageBase`，去尾斜杠）上；绝对 http(s)/data: url 原样透传。useGeneration 在 `saveGeneratedImage` 前调用。
- `saveGeneratedImage(url, id?)`：fetch → `arrayBuffer` → 分块 `String.fromCharCode` + `btoa` 转 base64（mime 取 `content-type` 头，缺省 `image/png`）→ put 进 IDB → 解析为结果 union `{ ok: true, id } | { ok: false, error }`（`id` 缺省时随机新 id；传入时存该 id——稳定 id 调用方 check-then-store 不重复抓取）；**任何失败**（fetch/CORS/网络/非 2xx/IDB 不可用）`console.error` 后带错误解析——非 2xx 的错误是 `HttpError`（带状态码，UI 据此走红浮窗友好文案），其余透传原始错误（UI 走 `imageSaveFailed` 错误行）——无原始 URL 回退，存不进库的 ref 视为不存在，永不抛错。
...
- `imageStore.spec.ts`：纯函数单测（`fake-indexeddb` 内存 IDB，每测试新 factory + `jest.resetModules` 隔离模块级单例；stub fetch 的 `arrayBuffer`，注意 Node Buffer 池——须 `new Uint8Array(Buffer)` 拷出；注意 resetModules 后 `instanceof HttpError` 断言须重新 `require` requestError，顶层 import 的类已过期）——`newImageId` 格式/唯一、`resolveGeneratedImageUrl`（绝对/data 透传；根相对 `image_proxy` 路径与相对路径拼端点 base、尾斜杠容忍）、显式稳定 id 存储（check-then-store 调用方）、成功存 data URI 后按 id 查回、fetch 失败/非 2xx/IDB 不可用三路径均解析为 `{ ok: false, error }`（无 URL 回退；非 2xx 的 error 是带 status 的 `HttpError`）、URL 样式值（https/data/根相对路径 `/…`）不再透传 → null、未命中 id → null。
- `requestError.spec.ts`：纯函数单测——`HttpError` 保留 status；`TypeError('Failed to fetch')`/各浏览器网络措辞（NetworkError…/Load failed）→ `unreachable`；401/403→`auth`、404→`notFound`、400/422→`rejected`、500/503→`server`、304→`unexpected`；`SyntaxError`→`unexpected`；`AssetLoadError`/普通 Error/字符串/null→`null`；`requestErrorMessage` 的 t() 调用序列（先服务名 key、后类别 key + `{service, status?}` 变量）、非 unreachable 才带 status、不泄漏原始错误文字；`HttpError.detail` 保留、有 detail 时 `requestErrorMessage` 原样返回它（零 i18n 查询）；`upstreamErrorMessage`（官方 `{"error": "…"}` 字符串提取 + trim，空串/纯空白/非字符串/对象形态/非 JSON/空体 → `null`）。
- `exampleCollection.spec.ts`：纯函数单测（stub fetch + `fake-indexeddb`）——`loadExampleCollection` 抓图持久化进 IDB（稳定 id `img-<文件名>`，成功时 `error=null`）、已在库不重复抓取、非法条目过滤、单张图失败 `imageId=null` 不影响其余且**静默**（`error=null`）、非 2xx（`error` 为带 status 的 `HttpError`）/非数组/网络失败回退空 entries + error 不抛错；`sizeFromRatio` 各比例换算与非法回退方形；`exampleToDesign` 的 id 取自 url 文件名、`images=[imageId]`（持久化失败 → `[]` 无 tile）、`rawPrompt`、size 随 `aspect_ratio`、无文件名回退 `example-<index>`。
- `resolutions.spec.ts`：纯函数单测——`OFFICIAL_RESOLUTIONS` 恰为 36 个官方固定分辨率（无重复，与需求列表逐项比对）、`PRESET_RATIOS` 不变；`ratioLabel` gcd 约分（16:9/1:1/9:23/9:22/3:8/2:3/3:1/4:3 + 0 输入回退 1:1）；分组 23 组且覆盖全部 36 个、组内每个成员的约分标签=组标签、组顺序比值严格递减（首 3:1、末 1:3）、组内大者在前（16:9=[2560x1440,1280x720]、1:1=[2048x2048,1024x1024]、9:23=[1152x2944]、1:3=[1024x3072,512x1536]）；`pickOfficialSize` 精确匹配、同比例最近面积（16:9 小→1280x720 / 大→2560x1440、4:3 默认 1024x768→1152x864、9:16→720x1280）、无比例匹配→最近 1:1（700x400→1024x1024）、0×0/0×500 安全。
...
- UI 布局类改动用 Playwright 对着运行中的 `expo start --web`（默认 8081 端口）做 e2e 验证（截图 + 几何断言）；i18n 专项为 `scripts/e2e_i18n.cjs`（en-US 默认 → 切 zh-CN 全页中文 → 刷新持久化 → 切回 en-US，含元素 desc/text 不翻译断言；首页断言含侧栏导航/墙标题/空态、prompt 占位符、圆形开始按钮 aria-label）；首页示例墙专项为 `scripts/e2e_example_wall.cjs`（读磁盘 example.json 驱动断言：Home 默认墙=示例集 tile 数=文件长度且各渲染示例图（IDB 解析出的 data URI，非打包文件 URL）、标题 "Collections"（"Recent Designs" 仅侧边栏出现一次）；悬停叠加原始 prompt；点击 tile 开全新设计——标题=HLD/画布比例=示例 aspect_ratio/参考图/Generated(1)/元素框数/Show Prompt 双栏/返回复位；S4 route.abort example.json → 红浮窗含 "example collection could not be loaded" + 无 tile + `noExamples` 空态，无 pageerror）；图片墙专项为 `scripts/e2e_home_wall.cjs`（Home 模式标题 "Collections" 且墙=示例集；Recent 模式标题 "Recent Designs" 出现、masonry 仅含有图设计且 DOM 顺序最新在前、tile 宽高比跟随设计尺寸、悬停叠加 rawPrompt/无 rawPrompt 回退 HLD、点击进入设计、返回复位 Home、无设计时空态）；显示 Prompt 专项为 `scripts/e2e_show_prompt.cjs`（按钮位于 Download 右侧；带/不带 rawPrompt 的已存设计经图片墙 tile 进入、handoff 新设计的左右两栏内容、只读、X 关闭、无数据时禁用）；图片持久化专项为 `scripts/e2e_image_store.cjs`（IDB 设计打开显示、生成存 base64 随机 id（按生成 data URI 过滤——首页加载已把示例图持久化进同一 IDB）、Save 存 id 后 reload 仍显示、转换抓取 500 → 无画布图 + 红浮窗含 `status 500`（无内联错误行、旧 `imageSaveFailed` 文案不出现）+ 不进历史 + 无新增 IDB 记录 + 保存的设计无图、旧 URL ref 不渲染图且 URL 永不被 fetch、首页墙 IDB tile 出 data URI / 旧 URL tile 保留但空占位、下载文件名、S8 生成请求 abort → 红浮窗 "Network problem" + "image generation service"（无 "Failed to fetch"、无内联错误行））；首页提交错误专项为 `scripts/e2e_start_design_error.cjs`（LLM 端点未设置正确时点击提交：**请求失败走红浮窗 `error-float`**——连接拒绝显示 "Network problem"（无 "Failed to fetch"、红底 rgb(229,57,53)、无内联错误行）且 ~6 秒内自动消失、修正 localStorage 端点后再次提交即跳转 /design；HTTP 500 显示 "temporarily unavailable (status 500)"（无 "LLM API Error"/"boom"）且恰 1 次 LLM 调用；HTTP 401 显示 "Settings problem" + "credentials"（status 401）——配置问题措辞；**非请求失败仍走红色错误行 `refine-error`**——空端点显示 "Missing settings: LLM endpoint" 且 0 次 LLM 请求；3 次全部返回非法 JSON 后显示 "…on every attempt" 且恰 3 次调用；全程无跳转、无 pageerror）；prompt 参考图拖入专项为 `scripts/e2e_prompt_image.cjs`（合成 `DragEvent` 向 `prompt-dropzone` drop PNG → W/H 行与输入条之间出现 48×48 预览（渲染该 data URI）、再 drop 追加、`X` 删除后列表重排；非 image/* 文件被忽略；dragover 输入条边框变蓝、离开还原；带参考图提交 → 请求体 `messages[1].content` 为数组（text 部分 + image_url data URI 部分）并跳转 /design；无参考图 → content 为纯字符串；粘贴 S6：合成 `ClipboardEvent` 图片粘贴 → preventDefault + 预览、纯文本粘贴不消费且无预览、真实 Ctrl+V（grantPermissions + Clipboard API）文本插入照常、图片粘贴出预览——系统剪贴板会重编码 PNG 字节，故断言**解码像素**相同而非原始字节）；背景编辑专项为 `scripts/e2e_background_edit.cjs`（seed 带 background 的设计直开 /design?id：点 background 块弹预填对话框（标题 Edit background、点遮罩关闭且不改数据）；Save 写回且标签更新、Save 设计把新 background 持久化进 localStorage；撤销栈一步（Undo 恢复原文）；纯空白草稿 Save 禁用（aria-disabled/pointer-events）、Cancel 不改原文）；首页输入多行专项为 `scripts/e2e_multiline_input.cjs`（输入框为 textarea、Enter 换行、单行条高 56px、3 行展开 ~83px、40 行封顶 200px 且内部可滚（scrollHeight>clientHeight）、清空缩回 56px）；官方分辨率选择器专项为 `scripts/e2e_official_resolution.cjs`（seed 设置 imageProvider=Official：S1 默认 23 个比例 pill + 无 W/H 输入 + 默认 4:3 1024x768 吸附到 1152x864（4:3 组 [2304x1728, 1152x864]、1152x864 激活、2304x1728 未激活）；S2 点 16:9 → 第二行=[2560x1440, 1280x720] 且较大者激活；S3 点 1280x720 + 提交 → LLM 请求体含 "TARGET IMAGE ASPECT RATIO: 16:9" 且 handoff size=1280x720（恰 1 次 LLM 调用，mock vLLM）；S4 设置对话框切 Custom 保存 → 预设 pill 行 + W/H 输入回来、1152x864 保留在输入框、比例落 'custom'（不被 1024 基准重置）、W/H 无联动；S5 再切回 Official → W/H 输入消失、1280x720 是合法官方分辨率被保留为激活项；全程无 pageerror。注意 RN-web 静态样式编译为 CSS 类，pill 激活态断言用计算样式 backgroundColor===rgb(0,122,255)；行容器不要加与 pill 前缀冲突的 testID（`official-ratio-row` 会匹配 `^="official-ratio-"`））；GitHub Pages 部署专项为 `scripts/e2e_github_pages.cjs`（`EXPO_BASE_URL=/DrawBoard` 真实导出——需 8081 空闲——+ bootstrap 注入 + 进程内 GH-Pages 模拟服务器 :8082（dist 挂 `/DrawBoard/`、缺失路径 status 404 回 404.html）：导出为 SPA 形态（index.html 在、无 per-route SSG 文件——`web.output: "single"`，`"static"` 会让深链 hydration mismatch #418 白屏）、首页全部 script src 带 base 前缀、示例墙全量出图（带前缀的 example.json/png fetch 在真实导出中生效）、深链 `/DrawBoard/design?id=…` 走 404 重写链后 URL 还原且设计页打开（标题=HLD、元素渲染）、未知路径 404→URL 还原无 pageerror、dist/404.html `pathSegmentsToKeep = 1` 与 .nojekyll 齐备、注入幂等）；图片生成端点专项为 `scripts/e2e_cf_worker.cjs`（设置对话框：Official 端点**硬编码** `/v1/ideogram-v4/generate`（无前缀）只读显示、Custom 可编辑（testID `settings-image-endpoint`）、来回切换存储值不被覆盖、填 worker URL 保存后持久化；Custom + worker 端点 → generate 请求发往 `<worker>/v1/ideogram-v4/generate`（`Api-Key` 头、multipart 体不变）且图持久化渲染；worker 回根相对 `image_proxy` URL（真实 cf-worker 的重写形态）→ 图片从 worker origin 抓取、不到 app origin 且持久化渲染；Official → 恒请求裸路径 `/v1/ideogram-v4/generate`（无前缀、解析到应用自身源，存储端点被忽略）；worker 返回 401 → 红浮窗 "Settings problem" + `status 401`、不泄漏上游响应体；worker 回 422 + 官方 `{"error": "…"}` 体（字符串形态，如安全审核文案）→ 红浮窗**原样显示该 error 内容**（替换通用文案、无 "Settings problem"、无状态码字样、无画布图——对象形态 `{"error": {"message"}}` 仍走通用文案，见 requestError.spec 用例）。注意 page.route 拦截会吞掉 CORS 预检 OPTIONS——真实 worker 的预检/转发/图片代理（`/v1/ideogram-v4/image_proxy`，生成响应里 `data[].url` 被 worker 重写为它）行为在 cf-worker 项目自己的测试里覆盖）：函数体引用 e2e 脚本模块级变量会 ReferenceError，且只接受**单个** arg——handoff 种子须把 payload 作为单一对象参数传入。`drawboard.designs` 种子必须是**数组**（`JSON.stringify([design])`）——store 的 `loadDesigns` 对非数组值返回空数组，种子写成裸对象时设计页查不到数据、只渲染 "Canvas Area" 占位（无报错，易误判为路由问题）。另注意 RN-web 0.21 会把**静态** StyleSheet 编译为 CSS 类（inline `style` 属性为空），e2e 断言样式须用 testID + 计算样式或动态 inline 值。
