...
src/
  ...
  app/
    ...
    test/
      services/            jest 单测（PromptRefiner、IdeogramPrompt、settings、designStore、imageDownload、imageStore、publicAsset、requestError）
...
- `HttpError extends Error`：非 2xx 响应，`status` 字段保留状态码（message 保留原始细节仅供 console，不进 UI）。`PromptRefiner.chatCompletion`（LLM 非 2xx）、`imageDownload`（下载非 2xx）、`saveGeneratedImage`（抓取生成图非 2xx）、`loadExampleCollection`（example.json 非 2xx）都抛它。
...
- `imageStore.spec.ts`：纯函数单测（`fake-indexeddb` 内存 IDB，每测试新 factory + `jest.resetModules` 隔离模块级单例；stub fetch 的 `arrayBuffer`，注意 Node Buffer 池——须 `new Uint8Array(Buffer)` 拷出；注意 resetModules 后 `instanceof HttpError` 断言须重新 `require` requestError，顶层 import 的类已过期）——`newImageId` 格式/唯一、显式稳定 id 存储（check-then-store 调用方）、成功存 data URI 后按 id 查回、fetch 失败/非 2xx/IDB 不可用三路径均解析为 `{ ok: false, error }`（无 URL 回退；非 2xx 的 error 是带 status 的 `HttpError`）、URL 样式值（https/data/根相对路径 `/…`）不再透传 → null、未命中 id → null。
- `requestError.spec.ts`：纯函数单测——`HttpError` 保留 status；`TypeError('Failed to fetch')`/各浏览器网络措辞（NetworkError…/Load failed）→ `unreachable`；401/403→`auth`、404→`notFound`、400/422→`rejected`、500/503→`server`、304→`unexpected`；`SyntaxError`→`unexpected`；`AssetLoadError`/普通 Error/字符串/null→`null`；`requestErrorMessage` 的 t() 调用序列（先服务名 key、后类别 key + `{service, status?}` 变量）、非 unreachable 才带 status、不泄漏原始错误文字。
