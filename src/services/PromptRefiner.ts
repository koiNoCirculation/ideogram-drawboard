const API_URL = 'http://192.168.10.4:8000/v1/chat/completions';
const API_TOKEN = 'yiteng_liu_access';
const SYSTEM_PROMPT_PATH = 'assets/system_prompt.txt';

/**
 * Calls the LLM to generate a structured caption for an image description.
 *
 * @param prompt - The user's natural language idea for the image.
 * @param aspectRatio - The target aspect ratio in W:H format (e.g., "16:9").
 * @returns A promise that resolves to the LLM's content response.
 */
export async function refine(prompt: string, aspectRatio: string): Promise<string> {
  try {
    // 1. Load the system prompt from the assets directory
    const systemPrompt = await loadSystemPrompt();

    // 2. Construct the user prompt using the specified template
    const userPrompt = `TARGET IMAGE ASPECT RATIO: ${aspectRatio} (width:height).\nUser idea: ${prompt}`;

    // 3. Perform the fetch request to the OpenAI-compatible endpoint
    const response = await fetch(API_URL, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${API_TOKEN}`,
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        model: 'nvidia/Gemma-4-26B-A4B-NVFP4', // Using a standard model name; adjust if the server requires a specific one
        messages: [
          { role: 'system', content: systemPrompt },
          { role: 'user', content: userPrompt },
        ],
        temperature: 1.0, // Set to 0 for stable, structured JSON output
      }),
    });

    if (!response.ok) {
      const errorDetails = await response.text();
      throw new Error(`LLM API Error (${response.status}): ${errorDetails}`);
    }

    const data = await response.json();

    // 4. Validate and return the response content
    if (data.choices && data.choices.length > 0 && data.choices[0].message) {
      return data.choices[0].message.content;
    } else {
      throw new Error('Received an unexpected response structure from the LLM API.');
    }
  } catch (error) {
    console.error('[refine Error]:', error);
    throw error;
  }
}

/**
 * Helper function to load the system prompt from the specified asset path.
 *
 * Handles both Browser/Mobile (via fetch) and Node.js (via fs) environments.
 */
async function loadSystemPrompt(): Promise<string> {
  try {
    // Detect Node.js environment (e.g., during Jest tests)
    const isNode = typeof process !== 'undefined' && !!process.versions && !!process.versions.node;

    if (isNode) {
      // In Node.js, we use fs to read the file locally
      const fs = await import('fs');
      const path = await import('path');
      const absolutePath = path.resolve(process.cwd(), SYSTEM_PROMPT_PATH);
      return fs.readFileSync(absolutePath, 'utf8');
    }

    // In Browser/Mobile environment, we use fetch
    const response = await fetch(SYSTEM_PROMPT_PATH);
    if (!response.ok) {
      throw new Error(`Failed to fetch system prompt: ${response.status} ${response.statusText}`);
    }
    return await response.text();
  } catch (error) {
    console.error('[loadSystemPrompt Error]:', error);
    throw new Error(`Could not load system prompt from ${SYSTEM_PROMPT_PATH}. Please ensure the file exists and is accessible.`);
  }
}
